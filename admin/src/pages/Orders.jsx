...
const Orders = ({ token }) => {
  ...
  const statusHandler = async (event,orderId) => {
    try {
      const response = await axiosInstance.post('/order/status',{orderId,status:event.target.value},{headers:{token}})
        ...
        {orders.map((order, index) => (
          ...
          >
            ...
            <div>
              ...
              <p>Payemnt : {order.payment ? "Done" : "Pending"}</p>
            ...
            <select onChange={(e) => statusHandler(e,order._id)} value={order.status} className="p-2 font-semibold">
              ...
              <option value="Out of delivery">Out of dilivery</option>
