...
const Login = ({setToken}) => {
    ...
    const onSubmitHandler = async (e) => {
        try {
            e.preventDefault();
            const response = await axiosInstance.post('/user/admin-login',{email,password})
            if(response.data.success){
                localStorage.setItem("admintoken", response.data.data.admintoken);
                setToken(response.data.data.admintoken);
            }else{
  ...
  return (
    <div className="min-h-screen flex items-center justify-center w-full">
      <div className="bg-white shadow-md rounded-lg px-8 py-6 max-mw-md">
